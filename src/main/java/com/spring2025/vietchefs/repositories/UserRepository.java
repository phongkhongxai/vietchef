...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query("SELECT COUNT(u) FROM User u WHERE u.role.roleName = 'CHEF' AND DATE(u.createdAt) = :date AND u.isDelete = false")
    Long countNewChefsByDate(@Param("date") java.time.LocalDate date);

    @Query("SELECT COUNT(u) FROM User u WHERE u.role.roleName = 'CUSTOMER' AND DATE(u.createdAt) = :date AND u.isDelete = false")
